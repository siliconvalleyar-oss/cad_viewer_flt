...
Aplicación Flutter profesional para visualizar y editar archivos **CAD** (implementada):
- **`.dxf`** — DXF texto plano (AutoCAD/LibreCAD); **parser DXF propio** (R12 AC1009 y R2000 AC1015): LINE, CIRCLE, ARC, ELLIPSE, LWPOLYLINE (con bulge), POLYLINE pesada (VERTEX/SEQEND), TEXT, MTEXT, INSERT, POINT, HATCH, SPLINE, DIMENSION (con DIMSTYLE), 3DFACE; escritura con `DxfWriter` propio (R2000/R12)
- **`.dwg`** — DWG binario propietario; detección + guía de conversión (MVP); ODA File Converter planeado
...
| State management | Provider + ChangeNotifier (CadViewModel) |
| Renderizado | CustomPainter (`CadPainter`) + gestos `onScale*` (pan/zoom 2 dedos) |
| Parsing DXF | **Parser propio** (`parsers/dxf_parser.dart`, sin dependencias externas) |
| Escritura DXF | `DxfWriter` propio (R2000/R12) |
| Parsing DWG | Detección + guía (v1.0); ODA File Converter (futuro) |
| Edición | Patrón Command + `CommandStack` propio (undo/redo, 100) |
| File picking | file_picker 10.3.10 (compat AGP 9) |
| Storage | path_provider + shared_preferences (autoguardado) |
...
### 🟡 4.1 — Soporte DWG/DGN
No hay parser nativo en Dart. ODA File Converter (CLI local) planeado; DGN fuera de alcance. Ver ADR-0005/0008. En v1.0: detección + guía de conversión.

### ✅ 4.2 — `utils/` implementado
`coordinate_transform`, `geometry`, `units`, `aci_colors`, `file_helper` — todos completos.

### 🟡 4.3 — `cad_painter.dart` puede crecer
Separar dibujo por tipo de entidad en métodos privados; overlays ya en renderers separados (grid, axis, grip, snap, layer).

### ✅ 4.4 — Tests unitarios
50 tests verdes (modelos). Pendiente: tests de parser/writer round-trip contra `test/files/` (ver `docs/TESTING.md`).
...
### 🔧 Renderizado de cotas (DIMENSION)
1. `CadDim` lleva x1/y1 (def), x2/y2 (texto), x3/y3 (ext 1, 13/23), x4/y4 (ext 2, 14/24), `textHeight` (dimtxt 140), `arrowSize` (dimasz 41), `measurement` (42)
2. El parser lee la tabla DIMSTYLE (140/41) y resuelve el estilo por grupo 3
3. `_paintDimension` (cad_painter): texto proporcional (dimtxt o 4% de la longitud, mínimo 12 px), flechas rellenas, etiqueta con `formatLength` en la unidad de visualización
